...
PRO THEMIS_IMAGER_READFILE,filename,images,metadata,COUNT=n_frames,DEBUG=debug,ALL_METADATA=all_metadata,STRIDE=stride
  ON_ERROR,0
  ...
  nchunk= 20 & nstart= (nchunk*nfiles/stride)<2400

  n_frames= 0  &  n_bytes= 0  &  _n_frames= 0
    ...
    WHILE NOT EOF(lun) DO BEGIN
     ...
     ; IF THEMIS_IMAGER_PNM_READFILE(lun,image,comments,DEBUG=debug) THEN BREAK
      ...
      IF (n_frames GT 0) THEN BEGIN
       IF (mdata.site_uid EQ '') THEN mdata.site_uid= metadata[n_frames-1].site_uid
       IF (mdata.imager_uid EQ '') THEN mdata.imager_uid= metadata[n_frames-1].imager_uid
      ENDIF
